...
CLUSTER_ARTIFACT_NAME = (
    "mindsync-cluster-averages"  # Match cluster training artifact name
)
ARTIFACT_VERSION = os.getenv(
    "ARTIFACT_VERSION", "latest"
)  # 'latest' or specific version like 'v0', 'v1'

CLUSTER_LABELS = ["dangerous", "not_healthy", "average", "above_average", "healthy"]
    ...
    try:
        ...
        if Path(artifact_dir).exists() and str(artifact_dir) != str(output_path):
            try:
                ...
                print(
                    f"🗑️  Cleaned up temporary download folder: {Path(artifact_dir).name}"
                )
...
def download_cluster_artifacts(output_dir="artifacts"):
    """
    Download cluster average CSV artifacts from Weights & Biases.
    ...
    try:
        print("\n" + "=" * 60)
        print("☁️  Downloading cluster average artifacts from W&B...")
        print("=" * 60)

        api = wandb.Api()

        if WANDB_ENTITY:
            artifact_path = f"{WANDB_ENTITY}/{WANDB_PROJECT}/{CLUSTER_ARTIFACT_NAME}:{ARTIFACT_VERSION}"
        else:
            artifact_path = (
                f"{WANDB_PROJECT}/{CLUSTER_ARTIFACT_NAME}:{ARTIFACT_VERSION}"
            )
        ...
        print(f"✅ Cluster artifact downloaded to: {artifact_dir}")

        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        print(f"📂 Copying cluster CSV files to {output_dir}/...")
        files_copied = 0
        for file in Path(artifact_dir).glob("*.csv"):
            dest = output_path / file.name
            shutil.copy2(file, dest)
            print(f"  ✓ {file.name}")
            files_copied += 1

        print(f"\n✅ Successfully copied {files_copied} cluster CSV files")
        ...
        print(f"❌ Error downloading cluster artifacts: {e}")
        return False
...
def verify_artifacts(artifacts_dir="artifacts"):
    ...
    required_files = [
        ...
        *[f"{label}_cluster_avg.csv" for label in CLUSTER_LABELS],
    ]
...
def main():
    ...
    model_success = download_artifacts(str(artifacts_dir))

    # Download cluster average CSV artifacts
    cluster_success = download_cluster_artifacts(str(artifacts_dir))

    if not model_success or not cluster_success:
        print("\n⚠️ One or more artifact downloads failed from W&B")
