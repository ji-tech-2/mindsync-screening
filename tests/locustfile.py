...
class MindSyncUser(HttpUser):
    ...
    def create_prediction(self):
        ...
        with self.client.post("/v1/predictions/create", json=payload, headers=headers, catch_response=True) as response:
            if response.status_code == 200 or response.status_code == 201 or response.status_code == 202:
                response.success()
            else:
                # Jika gagal, tampilkan pesan error dari server untuk mempermudah proses debug
                response.failure(f"Failed! Status: {response.status_code}, Text: {response.text}")